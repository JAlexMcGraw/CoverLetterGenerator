from fastapi import FastAPI, Depends, File, UploadFile, Response, Form
...
async def cover_letter_generate(openai_api_key: Annotated[str, Form()], job_post_url: AnyUrl, resume: UploadFile = File(...)):
    ...
    # May need to give OPENAI_API_KEY to this class
...
# @app.post("/generate_cover_letter/")
# async def cover_letter_generate(required_docs = RequiredDocs):
#     resume = await process_pdf(required_docs.resume.filename, is_local_file=True)

#     cl = CoverLetterGenerator(resume_path=resume['text'],
#                               job_posting_url=required_docs.job_post_url)
    
#     cover_letter = await cl.generate_cover_letter()
